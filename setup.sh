...
aws rds create-db-subnet-group \
    ...
    --subnet-ids "ENTER YOUR OWN SUBNET IDS"

# 创建 RDS MySQL 数据库实例,命名为 mydb,db.t2.micro 类型,用户名 admin,密码为 abc123
aws rds create-db-instance --db-instance-identifier mydb --engine mysql --db-instance-class db.r6g.large --master-username admin --master-user-password admin12345678 --allocated-storage 100 --db-subnet-group-name mydbsubnetgroup --vpc-security-group-ids "ENTER YOUR OWN VPC SECURITY GROUP ID"
